#include<stdio.h> 
/*现要求单步执行该程序，在watch窗口中观察i,k,sum,n值。具体操作如下
○1设输入5，观察刚执行完"scanf("%d",&k);"语句时，sum、k的值是多少？
...
○5在fabonacci函数递归执行过程中观察参数n的变化情况，并回答为什么k、sum在fabonacci函数内部不可见？(先减后增）	*/
...
long fabonacci(int n)
{
    if(n==1 || n==2)
        ...
        return fabonacci(n-1)+fabonacci(n-2);        //每级递归的n是私有的(创建新的n） 
}
